      ...
      when 'choice' # When the answer is a single choice from a list of choices
        options.map do |option|
          ...
          <div class="form-check">
            <input class="form-check-input" type="#{answer.choices.where(veracity: 1).size > 1 ? 'checkbox' : 'radio'}" value="#{option.first}" name="answer[#{answer.id}][]" id="Check#{answer.id}-#{options.index(option)}">
      ...
      when 'veracity' # When the answer follows a true or false rule
        options.map do |option|
          ...
          <div class="form-check form-switch">
            <input class="form-check-input" type="checkbox" role="switch" name="answer[#{answer.id}][]" value="#{option.first}" id="Check#{answer.id}-#{options.index(option)}">
            <label class="form-check-label" for="Check#{answer.id}-#{options.index(option)}">
              #{option.first}
            </label>
          </div>
