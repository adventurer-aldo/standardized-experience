function DataSubject(props) {
  ...
  const sendTag = (tag, id) => {
    axios.patch(`/api/evaluables/${id}`, {authenticity_token: props.auth_token,
    evaluable: {tag: tag}}).then(response => updateSubjects())
  }

  const setFocus = (focus, id) => {
    axios.patch(`/api/evaluables/${id}`, {authenticity_token: props.auth_token,
    evaluable: {focus: focus}}).then(response => updateSubjects())
  }
  ...
  return (
    <div className="w-100">
      ...
      <div className="row row-cols-1 row-cols-md-2 g-4 mt-1 justify-content-center">
        ...
        stat={props.stat} sendTag={sendTag} setFocus={setFocus} />
      </div>
