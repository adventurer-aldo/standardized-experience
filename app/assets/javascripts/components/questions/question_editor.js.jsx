function QuestionEditor(props) {
  [questionImage, setQuestionImage] = React.useState();

  const handleImage = (arg) => {
    setQuestionImage(URL.createObjectURL(arg.target.files[0]))
  };

  const returnInterface = () => {
    if (props.data.questionTypes.filter((questionType) => ['open', 'caption', 'choice', 'veracity', 'formula'].includes(questionType)).length > 0) {
      return (
        <div>
          {/* Answers */}
        ...
        <div className="modal-dialog">
          <div className="modal-content">
            ...
            <div className="modal-body">
              <input className="form-control mb-1" type="file" id="image" name="question[image]" onChange={event => handleImage(event)} />
              <img src={questionImage} className="img-fluid my-0 mx-auto d-block" />
              <div className="form-check form-switch">
                <input className="form-check-input" type="checkbox" role="switch" id="usePreviousImage" name="usePreviousImage" />
                <label className="form-check-label" htmlFor="usePreviousImage">Use from previous question</label>
              </div>
              <div className="d-flex"><span className="p-2" style={{ whiteSpace: 'nowrap' }}>Specify ID:</span><input className="form-control" type='number' name='previous_id' /></div>
