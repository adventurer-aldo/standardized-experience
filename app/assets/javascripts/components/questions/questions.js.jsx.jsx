class DataQuestions extends React.Component {
  ...
  handleKeyDown(event) {
    if (event.altKey && event.key === '+') {
      ...
      this.handleClickButton('caption')
    } else if (event.altKey && event.key === '3') {
      this.handleClickButton('formula')
    } else if (event.altKey && event.key === '4') {
      this.handleClickButton('choice')
    } else if (event.altKey && event.key === '5') {
      this.handleClickButton('veracity')
    } else if (event.altKey && event.key === '6') {
      this.handleMatchEqualizer();
      this.handleClickButton('match')
    } else if (event.altKey && event.key === '7') {
      this.handleFillEqualizer();
      this.handleClickButton('fill')
    } else if (event.altKey && event.key === '8') {
      this.handleFillEqualizer();
      this.handleClickButton('table')
    } else if (event.altKey && event.key === '9') {
      this.handleClickParameter('strict')
    } else if (event.altKey && event.key === '0') {
      this.handleClickParameter('order')
    }
  }
  ...
  setDetails(id) {
    ...
    .then(response => {
      ...
      choices: response.data.choices, parameters: response.data.parameters, image: response.data.image, level: response.data.level, subject: this.state.subjects.filter(subj => subj[0] == response.data.subject)[0]}})
    })
  }
  ...
  render() {
    ...
    <div>
      ...
      <div className="border-bottom border-dark text-end" >{this.state.results} result{this.state.results == 1 ? '' : 's'}</div>
