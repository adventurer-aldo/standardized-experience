...
<body>
    <div id="page">
        ...
        <?php $stats = pg_fetch_assoc(pg_query("SELECT * FROM statistics"));
        $id_data = pg_fetch_assoc(pg_query("SELECT * from quizzes WHERE id=".$stats['last_quiz_id']." LIMIT 1"));
        $id = $stats['last_quiz_id'];
        $testing = pg_query("SELECT * FROM quizzes WHERE id=$id ORDER BY id DESC");
        $correct = 0;
        $questions_num = 0;
        $score = 0;
        if (pg_num_rows($testing) > 0) {
            while ($questions = pg_fetch_assoc($testing)) {
                ...
                $original = pg_fetch_assoc(pg_query("SELECT * FROM quiz WHERE question='".$questions['question']."'"));
